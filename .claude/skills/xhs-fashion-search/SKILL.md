...
参数说明：
- `--page-size 20`：多拉一些（大部分封面图是拼接图/多人照，过滤后通常只剩 8~12 张）
...
搜索结果中很多封面图是多套穿搭拼接图（一张图里有 4~9 套 look 的九宫格/多宫格），这类图片不适合虚拟试穿，必须过滤掉。

**过滤方法**：用 `Read` 工具（Claude Code）或视觉能力逐张检查下载的封面图，分类为：

- **适合试穿** — 单人穿搭照（一张图只展示一个人、一套搭配），全身或大半身均可
- **不适合试穿** — 以下任一情况：
  - 多图拼接 / 九宫格 / 多宫格（一张图里有多套穿搭并排或网格排列）
  - 多人合照（两人及以上）
  - 纯文字 / 纯产品平铺图 / 无人物
  - 图片过小或模糊看不清穿搭细节
...
1. 用 `Read` 工具批量读取所有下载的封面图（可并行读取加速）
2. 逐张判断，标记为「适合」或「不适合」
3. 只保留「适合」的 items，丢弃不适合的
4. **如果适合的不足 8 张**：用相同关键词加 `--page N` 翻页继续搜索（同样 `--page-size 20`），重复过滤，直到凑够 8~12 张适合的图片
5. 对最终保留的图片重新编号（从 1 开始连续编号）

> 提示：可以先快速浏览所有图再统一过滤，不需要每张单独跟用户确认。过滤过程对用户透明，只需在最终展示时告诉用户「帮你筛选了 N 张适合试穿的穿搭」。
