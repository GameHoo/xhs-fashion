...
import requests
from .api.xhs_api_v2 import XhsApiV2
import logging
...
xhs_api = XhsApiV2(cookie=xhs_cookie)

@asynccontextmanager
async def server_lifespan(server: FastMCP):
    """管理服务生命周期：启动时初始化浏览器，关闭时清理资源"""
    print("aaaa")
    await xhs_api.start()
    try:
        yield
...
mcp = FastMCP("小红书", port=args.port, lifespan=server_lifespan)
